...
const addContact = () => {
  form.value.contact.push({ label: '', url: '' });
};

const removeContact = (index) => {
  form.value.contact.splice(index, 1);
};
...
const openCreateForm = () => {
  ...
  form.value = { semester: `${currentYear}1`, name: '', group: '', role: '', contact: [] };
  formError.value = '';
  showForm.value = true;
};
...
const handleSubmit = async () => {
  ...
  try {
    const payload = {
      ...form.value,
      contact: form.value.contact.filter(c => c.label && c.url),
    };
